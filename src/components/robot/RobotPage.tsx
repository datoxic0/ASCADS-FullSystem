...
import { BOARDS, LANGUAGES, DEFAULT_FILES, INITIAL_JOINTS } from "./templates";
import CimWorkspaceVisualizer from "./CimWorkspaceVisualizer";
import RobotWorkspaceIDE from "./RobotWorkspaceIDE";
...
import RobotChassisBuilder from "./RobotChassisBuilder";
...
export default function RobotPage() {
  ...
  const [activeMainTab, setActiveMainTab] = useState<"visualizer" | "ide" | "ai" | "digital-twin" | "env-builder" | "chassis-builder">("visualizer");
  ...
  const [windows, setWindows] = useState({
    visualizer: { isOpen: true, zIndex: 1 },
    ide: { isOpen: true, zIndex: 2 },
    ai: { isOpen: true, zIndex: 3 }
  });
  const [nextZIndex, setNextZIndex] = useState(4);

  const bringToFront = (key: 'visualizer' | 'ide' | 'ai') => {
    setWindows(prev => ({
      ...prev,
      [key]: { ...prev[key], zIndex: nextZIndex }
    }));
    setNextZIndex(prev => prev + 1);
  };

  const toggleWindow = (key: 'visualizer' | 'ide' | 'ai') => {
    setWindows(prev => {
      const isOpen = !prev[key].isOpen;
      return {
        ...prev,
        [key]: { ...prev[key], isOpen, zIndex: isOpen ? nextZIndex : prev[key].zIndex }
      };
    });
      ...
      {activeMainTab === "digital-twin" ? (
        ...
        >
          ...
          <div className="flex-1 h-full min-w-0">
            {(!windows.visualizer.isOpen && !windows.ide.isOpen && !windows.ai.isOpen) ? (
              <div className="h-full w-full flex items-center justify-center text-slate-500 font-mono flex-col gap-4">
                ...
                <span>All panels closed. Use the activity bar on the left to open a view.</span>
              </div>
            ) : (

...
export default function CimWorkspaceVisualizer({
  ...
  const handleSolveKinematicsWithAI = async () => {
    ...
    setIkError(null);
    try {
      const prompt = `Solve Inverse Kinematics for a ${robotType} robot to reach target position (X:${cartesianX}, Y:${cartesianY}, Z:0). Provide the exact joint angles needed in degrees. Keep it concise.`;
      const context = {
        joints: joints.map((j) => ({
          id: j.id,
          name: j.name,
      ...
      const responseText = await askAI(prompt, context);
      
      if (responseText.startsWith("**Gemini AI Error:**") || responseText.startsWith("**OpenRouter AI Error:**")) {
        throw new Error(responseText);
      }

      setIkResponse(responseText || "No kinematics solution data returned.");
    } catch (err: any) {
      console.error(err);
      setIkError(
        err.message ||
    ...
    >
      ...
      <div className="relative flex-1 w-full min-h-[250px] sm:min-h-[300px] md:min-h-[380px] bg-[#0d0d0f] flex flex-col items-center justify-center p-1.5 group select-none border-b border-emerald-300 dark:border-white/5">
        ...
        >
          ...
          {/* Render custom EnvironmentBuilder objects */}
          {envObjects && envObjects.length > 0 && (
            <g id="custom-env-objects">
              {envObjects.map((obj) => (
                <g 
                  key={obj.id} 
                ...
                >
                  <rect
                    ...
                    width={obj.width}
                    height={obj.height}
                    fill={obj.type === 'wall' ? '#334' : obj.type === 'cnc' ? '#223' : obj.type === 'conveyor' ? '#1a1a1a' : '#443'}

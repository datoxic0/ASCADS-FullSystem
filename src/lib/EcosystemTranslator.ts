...
export class EcosystemTranslator {
  ...
  static analogToDigital(design: CircuitDesign): DigitalCircuit {
    ...
    design.components.forEach(comp => {
      ...
      if (compType === 'SWITCH_SPST' || compType === 'BUTTON') kind = 'INPUT';
      else if (compType === 'LED' || compType === 'LAMP') kind = 'OUTPUT';
      else if (compType === 'BATTERY' || compType === 'VOLTAGE_SOURCE') kind = 'CONST1';
      else if (compType === 'GND' || compType === 'GROUND') kind = 'CONST0';
      else if (compType === 'LOGIC_AND') kind = 'AND';
      else if (compType === 'LOGIC_OR') kind = 'OR';
      else if (compType === 'LOGIC_NOT') kind = 'NOT';
      else if (compType === 'LOGIC_XOR') kind = 'XOR';
